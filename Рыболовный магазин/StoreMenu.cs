...
namespace FishingStore
{
    public class StoreMenu
    {
        ...
        public void ProcessPurchase()
        {
            ...
            Dictionary<FishingProduct, int> cart = new Dictionary<FishingProduct, int>();
            ...
            else if (choice == "2")
            {
                ...
                if (int.TryParse(Console.ReadLine(), out int setId))
                {
                    ...
                    {
                        // Предполагаем, что FishingSet имеет метод GetItems() для доступа к items
            ...
            // 6. Рассчитать стоимость со скидкой (если бонусы)
            decimal discount = Math.Min(customer.BonusPoints / 10m, 50); // до 50% скидки
            decimal finalPrice = totalPrice * (100 - discount) / 100;
            ...
            // 8. Начислить бонусы
            customer.AddBonusPoints((int)(finalPrice / 10));
            
            // 9. Выдать чек
            Console.WriteLine("\n=== ЧЕК ПОКУПКИ ===");
            Console.WriteLine($"Номер покупки: {manager.GetNextPurchaseNumber()}");
